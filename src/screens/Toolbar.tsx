  ...
  return (
    <div className="toolbar" onClick={(e) => e.stopPropagation()}>
      ...
      >
        ...
        <span className="toolbar-btn-label">{t('toolbar.edit')}</span>
      </Button>

      {isPdf && tool('merge', <Combine size={15} strokeWidth={2.75} />, t('toolbar.merge'))}
      {isPdf && tool('split', <Columns2 size={15} strokeWidth={2.75} />, t('toolbar.split'))}
      {tool('compress', <Minimize2 size={15} strokeWidth={2.75} />, t('toolbar.compress'))}

      <Button className="toolbar-btn" onClick={actions.rotateActivePage} disabled={busy} title={t('toolbar.rotateTitle')}>
        <RotateCw size={15} strokeWidth={2.75} />
        <span className="toolbar-btn-label">{t('toolbar.rotate')}</span>
      </Button>

      {tool('watermark', <Droplet size={15} strokeWidth={2.75} />, t('toolbar.watermark'))}
      {state.doc.pages.length > 1 && tool('reorder', <AlignJustify size={15} strokeWidth={2.75} />, t('toolbar.reorder'))}

      <div className="toolbar-spacer" />

      <Button icon disabled={!state.history.length || busy} title={t('toolbar.undo')} onClick={actions.undo}>
        <Undo2 size={15} strokeWidth={2.75} />
      </Button>
      <Button icon disabled={!state.future.length || busy} title={t('toolbar.redo')} onClick={actions.redo}>
      ...
      <div className="toolbar-zoom">
        ...
        <input
          ...
          title={t('toolbar.zoomTitle', { pct: Math.round(state.effectiveZoom * 100) })}
          aria-label={t('toolbar.zoomAria')}
        />
